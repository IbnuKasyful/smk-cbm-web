...
// NOTE: as of 2026-07-31 only `bisnis-digital` resolves to a real post on the
// Hostinger CMS (id 105, category "Program Keahlian"). The other five slugs
// below point at posts that do not currently exist on that install — they
// silently 404 in fetchProgramPosts()'s lookup and every one of those pages
// still renders from mock data despite the mapping suggesting otherwise.
...
// Every news post, newest first — backs the /news/arsip archive, which filters
// and paginates in-memory so a category switch never re-hits the CMS.
// WordPress caps per_page at 100, so larger archives need several requests;
// the page cap bounds the work if the CMS ever grows well beyond today's size.
const ARCHIVE_PAGE_SIZE = 100;
const ARCHIVE_MAX_PAGES = 5;

export async function getAllPosts() {
  if (!wpConfigured()) return POSTS;
  try {
    const exclude = excludeParam(await resolveCategories());
    const all = [];
    for (let page = 1; page <= ARCHIVE_MAX_PAGES; page += 1) {
      let data;
      try {
        data = await wpFetch('/wp/v2/posts', {
          params: {
            per_page: String(ARCHIVE_PAGE_SIZE),
            page: String(page),
            ...exclude,
            _embed: '1',
          },
        ...
        // WordPress answers 400 for a page past the end, which happens when the
        // post count is an exact multiple of the page size. Only treat that as
        // a failure if we have nothing to show yet.
        if (!all.length) throw err;
        break;
      }
      all.push(...data.map(normalizePost));
      if (data.length < ARCHIVE_PAGE_SIZE) break;
    }
    return all.length ? all : POSTS;
  } catch (err) {
    console.warn('[wordpress] getAllPosts fell back to mock data:', err.message);
    return POSTS;
  }
}
...
export async function getProgram(slug) {
  ...
  if (!wpSlug) return base; // slug we deliberately keep on mock prose
  try {
    const data = await wpFetch('/wp/v2/posts', {
