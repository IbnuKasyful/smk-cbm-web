...
// Category holding the CFA (Competition for Achievement) posts. These drive the
// CFA page and are likewise kept out of the news feed.
const CFA_CATEGORY_ID = 13; // "CFA"

// Categories that must never appear in the general news feed.
const NEWS_EXCLUDED_CATEGORIES = [PROGRAM_CATEGORY_ID, CFA_CATEGORY_ID].join(',');
  ...
  try {
    const data = await wpFetch('/wp/v2/posts', {
      params: {
        ...
        categories_exclude: NEWS_EXCLUDED_CATEGORIES,
        _embed: '1',
      },
    });
...
export async function getCfaItems() {
  ...
  try {
    const data = await wpFetch('/wp/v2/posts', {
      params: {
        per_page: '100',
        categories: String(CFA_CATEGORY_ID),
        _embed: '1',
      },
    });
    // Show exactly what's in the CFA category — an empty category yields an
    // empty list (the page renders an empty state), not mock content.
...
export async function getCfaItem(slug) {
  ...
  try {
    const data = await wpFetch('/wp/v2/posts', {
      params: { slug, categories: String(CFA_CATEGORY_ID), _embed: '1' },
    });
    // Only posts that live in the CFA category resolve here; anything else 404s.
